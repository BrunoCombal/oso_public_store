...
path=/data/public_store/lmes_shelf_area
rsrcDir=${path}/rsrc
outdir=${path}/
tmpdir=${path}/tmp
mkdir -p ${tmpdir}
...
do
    cp ${path}/lmes_shelf_area.${iext} ${tmpdir}
done

# copy information files
# rule: all files last change must be from the same day. Else, require the user to update them
memoDay='None'
for ii in readme.txt version.txt productDescription_lmes_shelf_area.txt lmes_shelf_area_xml_iso_19139.xml
do
    if [ ! -e "${ii}" ]; then
...
if [ -n "${version}" ]; then
    archiveName=lmes_shelf_area_${version}.zip
else
    archiveName=lmes_shelf_area.zip
fi
