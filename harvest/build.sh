...
function getXML(){
	...
	do
	    ...
	    listXML=(`find ${explore} -type f -iname "*iso*19139*xml" | grep -v -i 'new_' | grep -v -i 'old_' `)
	    for jj in ${listXML[@]}
	    do
		echo "Getting "$jj
